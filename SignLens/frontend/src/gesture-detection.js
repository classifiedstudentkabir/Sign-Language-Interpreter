...
class ImprovedGestureDetector {
  ...
  detectThumbsDown(landmarks) {
    ...
    const thumbTip = landmarks[LANDMARKS.THUMB_TIP];
    const thumbPip = landmarks[LANDMARKS.THUMB_PIP];
    const thumbDown = (thumbTip.y - thumbPip.y) > 0.03; // Lowered from 0.05 for better detection
   ...
   *   - Thumb curled
   * Mutual Exclusivity:
   *   ✅ Different from CALL_ME: this includes index
   *   ✅ Different from PEACE: this includes pinky instead of middle
   */
  detectRock(landmarks) {
    // REMOVED - User doesn't need this gesture
    return null;
  }
